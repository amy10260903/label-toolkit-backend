...
class MicrophoneRecognizer(BaseRecognizer):
    ...
    def __init__(self, dejavu):
        ...
        self.device = MicrophoneRecognizer.default_device
        self.chunksize = MicrophoneRecognizer.default_chunksize
        self.samplerate = MicrophoneRecognizer.default_samplerate
        self.recorded = False

    def select_device_idx(self):
        print("* select device")
        for i in range(self.audio.get_device_count()):
            dev = self.audio.get_device_info_by_index(i)
            print((i,dev['name'],dev['maxInputChannels']))

            if dev['name'].find('麥克風'):
                self.channels = dev['maxInputChannels']
                self.device = i
                return self.channels, self.device

    def start_recording(self, channels=default_channels, device=default_device,
    ...
    def recognize(self, seconds=10):
        channels, device = self.select_device_idx()
        self.start_recording(channels=channels, device=device)
